...
function editMemberForm() {
    ...
    useEffect(() => {
        fetch(`http://localhost:3004/member/${id}`)
            .then(resp => resp.json())
            .then(data => {
                setValues({
                    ...values,
    ...
    const handleSubmit = () => {
        ...
        formData.append("zodiak", values.zodiac);
        formData.append("blood", values.blood);
        formData.append("height", values.height);
        formData.append("showall", values.showall);
        formData.append("status", values.status);
        formData.append("profile", values.profile);
        formData.append("jiko", values.jiko);
        formData.append("instagram", values.instagram);
        formData.append("tiktok", values.tiktok);
        formData.append("tweet", values.tweet);
        formData.append("showroom", values.showroom);
        formData.append("fanbase", values.fanbase);
          const idAsNumber = parseInt(id, 10)
          const idmodif = idAsNumber + 1
        try {
            axios
                .post(`http://localhost:3000/member/post`, formData)
                .then(resp => {
                    if (resp.data.message == "SUCCESS") {
                      // alert("berhasil add data")
                      location.href = `/member/update/${idmodif}`
                    } else{
                      alert('gagal')
                    }
                });
    ...
    return (
        <div className=" w-full z-40 h-screen duration-500  bg-white p-4">
            <div className="flex justify-between">
                ...
                <Link className="hover:rotate-90 duration-300" to="/member">
                    <AiOutlineClose size={30} />
                </Link>
            </div>

            <form
                className="md:pb-0 md:h-auto h-screen
      "
            >
                <div
                ...
                >
                    ...
                    <div className="lg:col-span-2 lg:h-full">
                        <Textarea
                            ...
                            variant="outlined"
                            onChange={e =>
                                setValues({ ...values, jiko: e.target.value })
                            }
                            value={values.jiko}
                        />
                    </div>
                    ...
                    <div
                        className="min-[360px]:text-lg md:text-xl lg:text-2xl font-poppins font-bold text-red-500 
          sm:text-xl
          sm:col-span-2
          lg:-mb-10
          lg:col-span-5
          "
                    ...
                    <div>
                        <TextField
                            ...
                            variant="standard"
                            onChange={e =>
                                setValues({ ...values, tiktok: e.target.value })
                            }
                            name="tiktok"
                            value={values.tiktok}
                            className="w-full"
                    ...
                    <div>
                        <TextField
                            ...
                            variant="standard"
                            onChange={e =>
                                setValues({ ...values, tweet: e.target.value })
                            }
                            name="twitter"
                            value={values.tweet}
                            className="w-full"
                    ...
                    <div>
                        <TextField
                            ...
                            onChange={e =>
                                setValues({
                                    ...values,
                                    fanbase: e.target.value
                                })
                            }
                            name="fanbase"
                            value={values.fanbase}
                            className="w-full"
                        />
                    </div>
